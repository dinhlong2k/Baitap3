    ...
    if(mysqli_num_rows($result) > 0){
        ...
        while($item = mysqli_fetch_assoc($result)) {
            ...
            if($_SESSION['role'] == 'admin'){
                $output .='
                    <td>
                        <a id="btn-edit-salary" class="settings edit-btn" title="Settings" data-toggle="modal" data-target="#ModalCenterUpdate" data-idedit='. $item['id'].'><i class="fas fa-wallet"></i></a>
                    </td>';
            }
            $output .='</tr>';
            $id++;
